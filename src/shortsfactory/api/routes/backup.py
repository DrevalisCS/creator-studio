...
from shortsfactory.services.backup import BackupError, BackupService
...
def _service(settings: Settings) -> BackupService:
    from shortsfactory.services.updates import _resolve_current_version
...
async def restore_backup(
    ...
    try:
        ...
        svc = _service(settings)
        try:
            return await svc.restore_backup(db, tmp, allow_key_mismatch=allow_key_mismatch)
